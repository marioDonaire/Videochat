class VideoChat {
	constructor(ko) {
		...
		this.ws.onmessage = function(event) {
			...
			if(data.type=="DENY") {
				window.alert(data.remitente + " ha rechazado la llamada");
			}
		}
	}
	...
	rechazarLlamada(remitente, sessionDescription) {
		...
		let msg = {
				type : "DENY",
				recipient: remitente,
				sesionDescription : sessionDescription
			};
			this.ws.send(JSON.stringify(msg));
			this.addMensaje("Respuesta de rechazo enviada al servidor de Signaling");
	}
